...
PR title: `feat(landing-pages): [ABC-90] add shared ProductCard component`

```markdown
## Description

Adds a **ProductCard** compound component under `(landing-pages)` that supports the card layouts from the migration Figma mocks ([variation 1](https://www.figma.com/design/example?node-id=1-1980), [variation 2](https://www.figma.com/design/example?node-id=25-6674)).
...
Landing pages need product comparison cards matching the migration designs, and the mocks show more than one card.
...
- **New import path** — Landing pages compose cards from the `ProductCard` namespace rather than building their own. Search's own `ProductCards` are untouched and stay separate.
...
Compare the Responsive and Compact stories against the two Figma variations linked above. Both should match at desktop and mobile widths.
```

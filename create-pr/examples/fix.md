...
PR title: `fix(auth): [ABC-110] allow api host in csp connect-src`

```markdown
## Description

The insurance quote form failed to submit on production.
Requests to the API were blocked by our Content Security Policy, which allowlisted the widget origin but not the API origin the widget calls on submit.
...
`connect-src` enumerates permitted origins explicitly, and the integration shipped with only the widget origin listed.
...
- **`connect-src` allowlist** — Adds the API origin alongside the widget origin already present.
...
- [ ] Open the insurance page and submit a quote request
- [ ] Confirm the API call returns 200 in the network tab
- [ ] Confirm the console logs no CSP violation
- [ ] Confirm the response's `content-security-policy` header lists both origins
```
